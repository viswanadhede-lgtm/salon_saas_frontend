...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (companyErr) {
      ...
      return new Response(
        JSON.stringify({
          error: "Database error during company lookup",
        }),
        {
          status: 500,
          headers: {
    ...
    if (planErr) {
      ...
      return new Response(
        JSON.stringify({
          error: "Database error during plan lookup",
        }),
        {
          status: 500,
          headers: {
    ...
    if (
      verifiedPhone.length === 12 &&
      verifiedPhone.startsWith("91")
    ) {
      verifiedPhone = verifiedPhone.substring(2);
    }

    // ── 16. DUPLICATE SUBSCRIPTION CHECK ────────────────────────────────────
    const { data: existingSubs, error: existingErr } =
      await supabaseAdmin
        .from("subscriptions")
        .select(
          "subscription_id, plan_id, billing_cycle, status, created_at"
        )
    ...
    if (existingErr) {
      console.warn(
        "Existing subscription lookup warning:",
        existingErr
      );
      ...
      return new Response(
        JSON.stringify({
          error:
            "Company already has an active or in-progress subscription.",
    ...
    const subscriptionPayload: Record<string, unknown> = {
      ...
      // Monthly = 120 billing cycles
      // Yearly  = 10 billing cycles
      total_count:
        normalizedCycle === "yearly" ? 10 : 120,
      ...
      notes: {
        ...
        // Used by the webhook to distinguish trial
        // subscriptions from paid subscriptions.
    ...
    if (!rzpResponse.ok || !rzpSubscription.id) {
      ...
      const errMsg =
        rzpSubscription.error?.description ||
    ...
    const subStartDate = rzpSubscription.start_at
      ? new Date(
        (rzpSubscription.start_at as number) * 1000
      ).toISOString()
      : (
        rzpSubscription.current_start
          ? new Date(
            (rzpSubscription.current_start as number) * 1000
          ).toISOString()
          : null
      );

    // For a trial, the first charge date is the end of the trial period.
    // Store it locally so the global auth guard can enforce the 7-day
    // trial window.
    // After invoice.paid, the webhook will replace this with the
    // paid billing period end date.
    const trialEndDate =
      is_trial && rzpSubscription.charge_at
        ? new Date(
          (rzpSubscription.charge_at as number) * 1000
        ).toISOString()
        : null;

    const subEndDate = rzpSubscription.current_end
      ? new Date(
        (rzpSubscription.current_end as number) * 1000
      ).toISOString()
      : trialEndDate;
      ...
      await supabaseAdmin
        ...
        .insert({
          ...
          razorpay_customer_id:
            (rzpSubscription.customer_id as string) ||
            null,

          remarks: is_trial
            ? "7-day trial subscription created"
    ...
    if (subInsertErr) {
      console.error(
        "CRITICAL: Subscriptions row insertion failed:",
        subInsertErr
      );
      ...
      return new Response(
        JSON.stringify({
          error:
            "Critical consistency error: subscription created with payment provider but failed to record locally.",
        }),
        {
          status: 500,
          headers: {
    ...
    if (payInsertErr) {
      console.warn(
        "Warning: Initial payments row insertion failed:",
        payInsertErr
      );
    }
