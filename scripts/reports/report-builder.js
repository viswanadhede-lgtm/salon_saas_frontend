...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (type === 'customers') {
    ...
    } else if (type === 'sales-product-sales') {
        const companyId = localStorage.getItem('company_id');
        const filterStart = document.getElementById('filterStartDate');
        const filterEnd = document.getElementById('filterEndDate');
        const filterBranch = document.getElementById('filterBranch');
        ...
        const loadProductSalesData = async () => {
            ...
            try {
                ...
                if (tRes.data && tRes.data.length > 0) {
                    const tRows = tRes.data.map(r => {
                        const dateText = r.date ? new Date(r.date).toLocaleString() : '—';
                        const itemName = r.product_name || r.item_name || '—';
                        const cat = r.category || '—';
                        const qty = Number(r.quantity || 0).toLocaleString();
                        const price = formatCurrency(r.unit_price || r.price || 0);
                        const total = formatCurrency(r.total_amount || r.revenue || 0);
    ...
    } else if (type === 'sales-top-products') {
        const companyId = localStorage.getItem('company_id');
        const filterStart = document.getElementById('filterStartDate');
        const filterEnd = document.getElementById('filterEndDate');
        const filterBranch = document.getElementById('filterBranch');
        ...
        const loadTopProductsData = async () => {
            ...
            try {
                ...
                if (sumData) {
                    const row = Array.isArray(sumData) ? sumData[0] : sumData;
                    if (row) {
                        data.kpi1.value = row.top_product || row.top_product_name || '—';
                        data.kpi2.value = formatCurrency(row.top_5_revenue || row.total_sales || 0);
                        data.kpi3.value = row.top_5_contribution !== undefined ? `${row.top_5_contribution}%` : '—';
                        data.kpi4.value = row.best_seller || row.best_selling_product || '—';
                    }
                } else {
                    data.kpi1.value = '—';
                    data.kpi2.value = '₹0';
    ...
    } else if (type === 'sales-top-services') {
        const companyId = localStorage.getItem('company_id');
        const filterStart = document.getElementById('filterStartDate');
        const filterEnd = document.getElementById('filterEndDate');
        const filterBranch = document.getElementById('filterBranch');
    ...
    } else if (type === 'bk-completed') {
        // ── LIVE: Completed Appointments Report ──────────────────────────────
        const companyId    = localStorage.getItem('company_id');
        const filterStart  = document.getElementById('filterStartDate');
        const filterEnd    = document.getElementById('filterEndDate');
        const filterBranch = document.getElementById('filterBranch');
        ...
        const loadCompletedData = async () => {
            ...
            try {
                ...
                // 2. Trend Bar Chart — x: booking_date, y: completed count
                ...
                // 3. Donut Chart — label: 'completed'/'not_completed', count
    ...
    } else if (type === 'bk-cancelled') {
        // ── LIVE: Cancelled Appointments Report ───────────────────────────────
        const companyId    = localStorage.getItem('company_id');
        const filterStart  = document.getElementById('filterStartDate');
        const filterEnd    = document.getElementById('filterEndDate');
        const filterBranch = document.getElementById('filterBranch');
        ...
        const loadCancelledData = async () => {
            ...
            try {
                ...
                // 2. Trend Bar Chart — x: booking_date, y: cancelled count
                ...
                // 3. Donut Chart — label: 'Cancelled'/'Active', count
                ...
                if (tRes.data && tRes.data.length > 0) {
                    const tRows = tRes.data.map(r => {
                        ...
                        const service  = r.service_name  || '—';
                        const staff    = r.staff_name    || '—';
                        const amount   = r.total_price != null ? formatCurrency(r.total_price) : '—';
