...
export const supabase = {
    ...
    auth: {
        ...
        // OAuth: redirects browser to Supabase OAuth endpoint
        ...
        // Parses session from URL hash (after OAuth callback) or from localStorage
        async getSession() {
            try {
                // 1. Try URL hash first (OAuth callback sets #access_token=...)
                ...
                if (storedToken) {
                    ...
                    if (res.ok) {
                        const user = await res.json().catch(() => null);
                        return {
                            data: { session: { access_token: storedToken, user }, user },
                            error: null
                        };
                    }
                }
    ...
    async rpc(fnName, params = {}) {
        try {
            const res = await fetch(`${this._url}/rest/v1/rpc/${fnName}`, {
                method: 'POST',
            ...
            return {
                async upload(path, file, options = {}) {
                    try {
                        ...
                        return { data: { Key: `${bucket}/${cleanPath}`, ...data }, error: null };
                    } catch (err) {
                        return { data: null, error: err };
                    }
                },
                ...
                async createSignedUrl(path, expiresIn = 3600) {
                    try {
                        ...
                        const res = await fetch(`${SUPABASE_URL}/storage/v1/object/sign/${bucket}/${cleanPath}`, {
                            method: 'POST',
                            headers,
                        ...
                        const signedUrl = data?.signedURL 
                            ? (data.signedURL.startsWith('http') ? data.signedURL : `${SUPABASE_URL}/storage/v1${data.signedURL}`)
                            : null;
...
class SupabaseQuery {
    ...
    upsert(data, { onConflict = null } = {}) {
        this._method = 'POST';
        this._headers['Prefer'] = 'resolution=merge-duplicates,return=representation';
        if (onConflict) {
            this._params.push(`on_conflict=${encodeURIComponent(onConflict)}`);
        }
        this._body = JSON.stringify(Array.isArray(data) ? data : [data]);
        return this;
    }
